$(document).ready(function () {
	// Function to move child div outside parent based on screen width
	var screenWidth = $(window).width();
	if (screenWidth <= 767) {
		// Detach the child from the parent and append it to the body
		$('.search-holder').detach().appendTo('.middle-nav .container');
	}
	...
	$(window).resize(function () {
		if ($(window).width() < 768) {
			...
			$('.search-holder').appendTo('.middle-nav-desc').insertAfter('.brand-holder');
		}
	});

	// Add/Remove class for sidebar menu
	$('.mobile-menu').click(function () {
		$('.main-header').toggleClass('show-menu');
	});

	// Add/Remove class for cart drawer
	$('.cart-holder a').click(function () {
		$('.cart-drawer-wrapper').addClass('show-drawer');
		$('body').addClass('show-overlay');
	});
	$('.hide-drawer').click(function () {
		$('.cart-drawer-wrapper').removeClass('show-drawer');
	...
	let days = Math.floor(Math.random() * 31);
	let hours = 23;
	let minutes = 59;
	let seconds = 59;
